...
| R-02 | JWT secret reuse across webapp auth and API token signing increases blast radius                                             | `apps/webapp/auth.config.ts` uses `JWT_SECRET`; API signs/verifies tokens with same secret in `apps/api/lib/utils/auth.ts`                                                                                | 4          | 5      | 20    | Critical |
...
### R-02: Single JWT secret compromise -> multi-surface auth compromise

A leak in webapp runtime environment or CI/CD logs can become API token-forging capability because the same secret
validates API auth tokens and webapp auth/session behavior.
...
No promotion to a new stage should proceed with unresolved Critical risks (`R-01`, `R-02`, `R-11`).
