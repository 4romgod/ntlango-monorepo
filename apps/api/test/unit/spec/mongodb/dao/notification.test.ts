...
import type {Notification, CreateNotificationInput} from '@ntlango/commons/types';
...
describe('NotificationDAO', () => {
        ...
        it('creates multiple notifications in bulk', async () => {
            ...
            expect(NotificationModel.insertMany).toHaveBeenCalledWith(
                expect.arrayContaining([
                    expect.objectContaining({isRead: false, emailSent: false, pushSent: false}),
                ]),
            );
    ...
    describe('readByUserId', () => {
        it('returns paginated notifications for a user', async () => {
            const mockNotifications = [
                {toObject: () => mockNotification, createdAt: new Date('2024-01-01T00:00:00Z')},
            ];

            (NotificationModel.find as jest.Mock).mockReturnValue(
                createMockSuccessMongooseQuery(mockNotifications),
            );
            (NotificationModel.countDocuments as jest.Mock).mockReturnValue(
                createMockSuccessMongooseQuery(1),
            );
        ...
        it('returns hasMore=true when more results exist', async () => {
            ...
            (NotificationModel.countDocuments as jest.Mock).mockReturnValue(
                createMockSuccessMongooseQuery(25),
            );
        ...
        it('filters by cursor when provided', async () => {
            ...
            (NotificationModel.countDocuments as jest.Mock).mockReturnValue(
                createMockSuccessMongooseQuery(0),
            );
    ...
    describe('countUnread', () => {
        it('returns count of unread notifications', async () => {
            (NotificationModel.countDocuments as jest.Mock).mockReturnValue(
                createMockSuccessMongooseQuery(5),
            );
    ...
    describe('markAllAsRead', () => {
        it('marks all notifications as read and returns count', async () => {
            (NotificationModel.updateMany as jest.Mock).mockReturnValue(
                createMockSuccessMongooseQuery({modifiedCount: 5}),
            );
    ...
    describe('delete', () => {
        it('deletes a notification and returns true', async () => {
            (NotificationModel.deleteOne as jest.Mock).mockReturnValue(
        ...
        it('returns false when notification not found', async () => {
            (NotificationModel.deleteOne as jest.Mock).mockReturnValue(
                createMockSuccessMongooseQuery({deletedCount: 0}),
            );
    ...
    describe('markEmailSent', () => {
        it('updates emailSent status', async () => {
            (NotificationModel.updateOne as jest.Mock).mockReturnValue(
